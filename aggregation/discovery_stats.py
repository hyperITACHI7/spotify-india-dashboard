...
def get_topics_matrix(date_range: str, version: str, rating: str, platform: str, search: str, mode: str = None) -> List[Dict[str, Any]]:
    ...
    for topic_id, topic_reviews in topic_buckets.items():
        ...
        if effective_mode == "snapshot" and topic_id in _MOCK_TOPIC_SUMMARIES:
            ...
            # Live mode: skip the topic_summaries DB cache — it may contain summaries
            # generated by older code versions with stale templates (e.g. "leading concerns").
            # Always compute fresh: LLM first, then rule-based text fallback.
